...
Future<Declaration> getDeclarationFromSearchPageData({
  ...
  final declarationDbId = await http
      .post(
        Uri.https(
          "www1.aade.gr",
  ...
  final DateTime arrivalDate = DateFormat("dd/MM/y").parse(getBetweenStrings(
      res.body, 'appForm:rentalFrom_input" type="text" value="', '"'));
  final DateTime departureDate = DateFormat("dd/MM/y").parse(getBetweenStrings(
      res.body, 'appForm:rentalTo_input" type="text" value="', '"'));
  final platform = DeclarationBody.extractBookingPlatform(getBetweenStrings(
    getBetweenStrings(res.body, 'id="appForm:platform_input"', '</select>'),
    'selected="selected">',
    '</option>',
  ));

  print(getBetweenStrings(
    getBetweenStrings(res.body, 'id="appForm:platform_input"', '</select>'),
    'selected="selected">',
    '</option>',
  ));

  final payout = double.tryParse(getBetweenStrings(
  ...
  if (payout != null && payout > 0) {
    ...
    final cancellationAmount = double.tryParse(getBetweenStrings(
