...
/// Configures how an [CustomAnimationController] behaves when animations are
/// disabled.
///
...
/// animations. This enum is used to allow certain [CustomAnimationController]s to opt
/// out of this behavior.
///
/// For example, the [CustomAnimationController] which controls the physics simulation
...
enum AnimationBehavior {
  /// The [CustomAnimationController] will reduce its duration when
  /// [AccessibilityFeatures.disableAnimations] is true.
  normal,

  /// The [CustomAnimationController] will preserve its behavior.
  ///
...
///
/// By default, an [CustomAnimationController] linearly produces values that range
...
/// ## Ticker providers
///
/// An [CustomAnimationController] needs a [TickerProvider], which is configured using
...
/// [CustomAnimationController] class uses a [Ticker] to step through the animation
/// that it controls.
///
/// If an [CustomAnimationController] is being created from a [State], then the State
...
/// single [CustomAnimationController] during its entire lifetime).
///
...
/// ## Life cycle
///
/// An [CustomAnimationController] should be [dispose]d when it is no longer needed.
/// This reduces the likelihood of leaks. When used with a [StatefulWidget], it
/// is common for an [CustomAnimationController] to be created in the
/// [State.initState] method and then disposed in the [State.dispose] method.
///
/// ## Using [Future]s with [CustomAnimationController]
///
...
///
/// {@tool dartpad}
/// This example shows how to use [CustomAnimationController] and
...
/// [CustomAnimationController.duration] to be reduced so that the visuals
/// don't fall behind.
///
...
/// See also:
///
///  * [Tween], the base class for converting an [CustomAnimationController] to a
///    range of values of other types.
class CustomAnimationController extends Animation<double>
    with
  ...
  CustomAnimationController.unbounded({
    double value = 0.0,
    this.duration,
    this.reverseDuration,
    this.debugLabel,
  ...
  void _maybeDispatchObjectCreation() {
    if (kFlutterMemoryAllocationsEnabled) {
      FlutterMemoryAllocations.instance.dispatchObjectCreated(
        ...
        className: "$CustomAnimationController",
        object: this,
      );
    }
  }
  ...
  /// pointer to mutate the [CustomAnimationController] state.
  ...
  /// [CustomAnimationController] when no explicit value is set for [min] and [max].
  ///
  ...
  /// [upperBound]. To avoid this, consider creating the [CustomAnimationController]
    ...
    assert(() {
      if (_ticker == null) {
        throw FlutterError.fromParts(<DiagnosticsNode>[
          ...
          DiagnosticsProperty<CustomAnimationController>(
